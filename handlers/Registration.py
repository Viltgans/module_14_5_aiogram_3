...
async def set_username(message: Message, state: FSMContext):
    with suppress(TelegramBadRequest):
        ...
        # Если в процессе регистрации пользователь выбрал кнопку 'Информация', вывод меню кнопки 'Информация'
        if message.text == "Информация":
            await repeat_reg.repeat_reg_info(message, state)
        # Если в процессе регистрации пользователь выбрал кнопку 'Рассчитать', вывод меню кнопки 'Рассчитать'
        if message.text == "Рассчитать":
            await repeat_reg.repeat_reg_calc(message, state)
        # Если в процессе регистрации пользователь выбрал кнопку 'Купить', вывод меню кнопки 'Купить'
        if message.text == 'Купить':
            await repeat_reg.repeat_reg_buy(message, state)
...
async def set_age_reg(message: Message, state: FSMContext):
    with suppress(TelegramBadRequest):
        ...
        # Если возраст не является целым числом
            ...
            if message.text == 'Купить':
                await repeat_reg.repeat_reg_buy(message, state)
            # Если возраст не является целым числом, вывод сообщения об ошибке
            else:
                # Удаление сообщения от пользователя с ошибкой ввода данных
                await message.delete()
                # Вывод сообщения об ошибке ввода данных
